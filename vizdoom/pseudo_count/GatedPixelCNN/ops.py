...
def gated_conv(inputs, kernel_shape, data_num_channels, scope="gated_conv"):
  with tf.variable_scope(scope):
    ...
    horiz_inputs, vert_inputs  = tf.split(axis=3, num_or_size_splits=2, value=inputs)  
    ...
    return tf.concat(axis=3, values=[horiz_outputs, vert_gated_out]) 
...
def _gated_activation_unit(inputs, kernel_shape, mask_type, data_num_channels, scope="gated_activation_unit"):
  with tf.variable_scope(scope):
    ...
    bd_out_0, bd_out_1 = tf.split(axis=3, num_or_size_splits=2, value=bd_out)
